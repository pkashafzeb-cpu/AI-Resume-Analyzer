...
<div class="feature-strip">
    ...
    <span class="pill pill-violet">🎯 ATS Friendly</span>
...
with st.sidebar:
    st.markdown(f"<p style='font-family:\"JetBrains Mono\",monospace; font-size:0.75rem; letter-spacing:0.14em; color:{PALETTE['cyan']};'>// SCAN SETTINGS</p>", unsafe_allow_html=True)
...
def run_analysis(resume_text, mode, target_role, job_description, comparison_text, temperature, max_tokens):
    ...
    sub_scores = extract_sub_scores(response.text)  # <-- New
...
with tab1:
    ...
    # Updated uploader to accept PDF, DOCX, TXT
    uploaded_file = st.file_uploader("Choose your resume (PDF, DOCX, or TXT)", type=["pdf", "docx", "txt"], help="Text-based PDF, DOCX, or TXT. Max 10MB.")
    ...
    if uploaded_file is not None:
        ...
        if analyze_button:
            ...
            # --- NEW: ATS Formatting Audit (only for PDF) ---
            if file_type == 'pdf':
            ...
            with st.spinner(f"🧠 Analyzing ({analysis_mode})..."):
                try:
                    # run_analysis now returns 4 values
            ...
            # --- NEW: Semantic similarity if JD provided ---
            if job_description:
...
with tab2:
    ...
    if run_sample:
        ...
        with st.spinner(f"🧠 Analyzing sample ({sample_mode})..."):
            try:
                analysis_text, score, max_score, sub_scores = run_analysis(
...
with tab3:
    ...
    if st.session_state.current_analysis and st.session_state.current_analysis.get("score") is not None:
        ...
        # Show stored sub-scores if available
